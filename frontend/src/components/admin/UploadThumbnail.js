...
export default function UploadThumbnail({value , setValue}) {
  const [publicId, setPublicId] = useState("");
  const [cloudName] = useState("dtur9xepq");
  const [uploadPreset] = useState("thumbnailUpload");
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <CloudinaryUploadWidget uwConfig={uwConfig} setPublicId={setPublicId} onUploadSuccess={handleUploadSuccess} label={"Thumbnail( < 2MB)"}/>
        <AdvancedImage
          style={{ maxWidth: "100%" }}
          cldImg={myImage}
